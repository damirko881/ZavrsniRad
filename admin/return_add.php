<?php
	...
	if(isset($_POST['add'])){
		...
		if($query->num_rows < 1){
			...
			$_SESSION['error'][] = 'Student nije pronađen';
		}
		else{
				...
				if(!empty($isbn)){
					...
					if($query->num_rows > 0){
						...
						if($query->num_rows > 0){
							...
							$_SESSION['error'][] = 'Podaci o posudbi nisu pronađeni: ISBN - '.$isbn.', ID studenta: '.$student;
						}
						...
						$_SESSION['error'][] = 'Knjiga nije pronađena: ISBN - '.$isbn;
					}
			...
			if($return > 0){
				...
				$_SESSION['success'] = $return.' '.$book.' Uspješno vraćena';
			}
